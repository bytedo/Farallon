...
<head>
    ...
    <?php wp_head(); ?>
    <link type="image/vnd.microsoft.icon" href="<?php echo esc_url($farallonSetting->get_setting('favicon') ? $farallonSetting->get_setting('favicon') :  get_template_directory_uri() . '/build/images/favicon.png'); ?>" rel="shortcut icon">
</head>
        ...
        <header class="fHeader">
            <a href="<?php echo home_url(); ?>" class="site--url" aria-label="<?php bloginfo('sitename'); ?>">
                <?php if ($farallonSetting->get_setting('logo')) : ?>
                    <img src="<?php echo esc_url($farallonSetting->get_setting('logo')); ?>" class="avatar" alt="<?php bloginfo('sitename'); ?>" />
