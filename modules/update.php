...
{
    add_thickbox();
    $nonce = wp_create_nonce('farallon_theme_update');
    echo '<div class="updated">
    <p>主题最新版为 ' . esc_html(get_transient('farallon_latest')) . '，当前版本 ' . esc_html(FARALLON_VERSION) . '。请备份好所有文件，主题升级过程中会删掉原有文件。<a class="thickbox" href="' . esc_url(admin_url() . 'admin-ajax.php?action=farallon_theme_update&nonce=' . $nonce . '&TB_iframe=true&width=772&height=312') . '">确认升级</a>
    </p></div>';
}
...
{
    // 安全修复：仅管理员可操作 + nonce 验证
    if (!current_user_can('update_themes')) {
        wp_die('权限不足', 403);
    }
    check_ajax_referer('farallon_theme_update', 'nonce');
