...
function farallon_get_post_image_count($post_id)
{
    ...
    // 轻量正则统计，跳过 the_content 过滤器（避免短代码/oEmbed 重量级处理）
    $content =  apply_filters('the_content', $content);
    preg_match_all('/<img.*?(?: |\\t|\\r|\\n)?src=[\'"]?(.+?)[\'"]?(?:(?: |\\t|\\r|\\n)+.*?)?>/sim', $content, $strResult, PREG_PATTERN_ORDER);
    $count = count($strResult[1]);
    $cache[$post_id] = $count;
...
function farallon_get_background_image($post_id, $width = null, $height = null)
{
    ...
    static $cache = array(); // 同页请求内缓存，避免重复计算

    $cache_key = $post_id . '|' . $width . '|' . $height;
    ...
    if (has_post_thumbnail($post_id)) {
        // 优先 medium_large 缩略图，避免 full 尺寸触发 image_downsize 全量查询
        $timthumb_src = wp_get_attachment_image_src(get_post_thumbnail_id($post_id), 'medium_large');
        if (!$timthumb_src) {
    ...
    // 图片尺寸裁剪：COS 图片追加腾讯云 imageMogr2 裁剪参数（域名在主题设置中配置）
    if ($height && $width) {
        $cos_domain = $farallonSetting->get_setting('cos_domain');
        if ($cos_domain && strpos($output, $cos_domain) !== false) {
            $output .= '?imageMogr2/thumbnail/' . $width . 'x' . $height;
        } elseif ($farallonSetting->get_setting('upyun')) {
            $output .= "!/both/{$width}x{$height}";
        } elseif ($farallonSetting->get_setting('oss')) {
            $output .= "?x-oss-process=image/crop,w_{$width},h_{$height}";
        } elseif ($farallonSetting->get_setting('qiniu')) {
            $output .= "?imageView2/1/w/{$width}/h/{$height}";
        }
    }
...
{
    static $has_image_cache = array();
    if (isset($has_image_cache[$post_id])) {
        return $has_image_cache[$post_id];
    }
    ...
    $has_image_cache[$post_id] = $has_image;
